...
export type ListChainflipAffiliatesResponse = {
    brokerAddress: string;
    capacity: {
        used: number;
        limit: number;
...
export type DeregisterChainflipAffiliateRequest = {
    key?: string;
...
export type DeregisterChainflipAffiliateResponse = {
    affiliateAccountId: string;
    shortId: number;
...
export type PruneChainflipAffiliatesRequest = {
    dryRun?: boolean;
    minIdleDays?: number;
    maxCount?: number;
    includeLinkedApiKeys?: boolean;
    updatedBy: {
        userId?: string;
    };
};

export type PruneChainflipAffiliatesResponse = {
    dryRun: boolean;
    capacity: {
        used: number;
        limit: number;
...
export type GetQuoteRequest = {
    ...
    /**
     * Address to send asset from. On a UTXO sell chain this may name several addresses to draw inputs from, separated by `;` — the first is used wherever a single address is required (refund address, memos, deposit channels). At most 6 addresses; repeats are ignored, empty entries are rejected.
     */
    sourceAddress?: string;
    /**
    ...
    refundAddress?: string;
    /**
     * How to react when a route cannot honour refundAddress: WARNING (default) keeps the route and reports refundAddressNotApplied in its warnings[], STRICT drops it instead. Ignored when refundAddress is absent.
     */
    refundAddressSetting?: 'STRICT' | 'WARNING';
    /**
...
export type ExecuteSwapRequest = {
    ...
    destinationAddress: string;
    /**
     * Address to refund to if the swap fails, on the SELL chain. Overrides the /v3/quote value; falls back to it, then to sourceAddress. See /v3/quote for per-provider behaviour.
     */
    refundAddress?: string;
    /**
     * How to react when the route cannot honour refundAddress: WARNING (default) proceeds and reports refundAddressNotApplied in the route's warnings[], STRICT rejects with refundAddressUnsupportedByRoute (the provider always refunds the sender) or refundAddressTooLongForSellChain (the provider carries it in a memo this sell chain has no room for). Inherited from the /v3/quote request when omitted.
     */
    refundAddressSetting?: 'STRICT' | 'WARNING';
    /**
...
export type GetGasPricesData = {
    ...
    query?: {
         ...
         */
        chainId?: '1' | '10' | '56' | '100' | '137' | '143' | '196' | '999' | '4663' | '5042' | '8453' | '36900' | '42161' | '43114' | '80094' | '728126428' | 'aleo' | 'bitcoin' | 'bitcoincash' | 'cardano' | 'cosmoshub-4' | 'dash' | 'dogecoin' | 'kaiyo-1' | 'litecoin' | 'mayachain-mainnet-v1' | 'near' | 'solana' | '0x534e5f4d41494e' | 'stellar' | 'sui' | 'ripple' | 'thorchain-1' | 'ton' | 'zcash';
        /**
